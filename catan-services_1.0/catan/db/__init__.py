...
class db_service_status(DBSchema):
    ...
    def sql_insert(self, service_id, submission_id):
        return self._get_sql_insert(service_id=service_id,
                                    submission_id=submission_id)

    _SQL_CREATE = '''CREATE TABLE db_service_status
            (submission_id int,
            service_id int,
            service_status int,
            service_comments text)'''

class db_announcements(DBSchema):
    submitter_name = None
    submitter_organization = None

    def sql_insert(self, announcement_id, timestamp):
        return self._get_sql_insert(announcement_id=announcement_id,
                                    timestamp=timestamp)

    _SQL_CREATE = '''CREATE TABLE db_announcements
            (announcement_id int,
            origin_node_id int,
            title text,
            content text,
...
class CatanDatabase:
    ...
    def __init__(self, node_id=-1, filename=G.DB_FILENAME):
        ...
        self.next_service_id = None
        self.next_announcement_id = None
    ...
    def __create(self):
        ...
        try:
            ...
            # Setup our services (Request/Volunteer) database
            c.execute(db_services._SQL_CREATE)

            c.execute(db_service_status._SQL_CREATE)

            # announcements
            c.execute(db_announcements._SQL_CREATE)
    ...
    def get_next_service_id(self, origin_node_id):
        """
            Return the next service ID to use
        """
        logger.debug("Getting next service ID.")

        # Let's not hit the disk if we don't have to
        if self.next_service_id is not None:
            logger.debug("Updating service ID in memory.")
            rtn = self.next_service_id
            self.next_service_id += 1
            return "%d-%d"%(rtn, origin_node_id)

        logger.debug("Updating service ID from disk.")
        # Query the database to see what the last id was?
        SQL = "SELECT service_id FROM db_services WHERE origin_node_id = ?"
        results = self._sql(SQL, [origin_node_id]).fetchall()

        # If it's empty, this is a new database
        if not results:
            self.next_service_id = 2
            return "1-%d"%origin_node_id
        else:
            id_list = [int(r[0].split("-")[0]) for r in results]
            id_list.sort(reverse=True)
    ...
    def get_next_announcement_id(self, origin_node_id):
        """
            Return the next announcement ID to use
        """
    ...
    def update_gps(self, node_message):
        ...
        db_obj = CatanDatabaseNodeObject(node_message.data)
        logger.debug(db_obj)
    ...
    def get_person_counts_by_node(self):
        ...
        # SQL = "SELECT ROUND(person_id*100)%100 as Node, COUNT(*) FROM
        # db_person_bio GROUP BY Node"
    ...
    def update_person(self, node_message):
        ...
        db_obj = CatanDatabasePersonObject(node_message.data)
        logger.debug(db_obj)
        ...
        if db_obj.data_picture.has_entries():
            ...
            picture_sql = db_obj.data_picture.sql_insert(picture_id,
                                                         origin_node_id)
    ...
    def update_service(self, node_message):
        """
            Update a service object
        """

        logger.debug("Got a database update message. (Request)")

        # Populate our DB Object
        from catan.db.services import CatanDatabaseServiceObject
        db_obj = CatanDatabaseServiceObject(node_message.data)
        logger.debug(db_obj)

        if node_message.source != 0:
            db_obj.origin_node_id = node_message.source
        else:
            db_obj.origin_node_id = self.my_node_id
        ...
        # Get our service ID
        if db_obj.service_id is None:
            db_obj.service_id = self.get_next_service_id(self.my_node_id)

        # Timestamp the message
        if db_obj.timestamp is None:
            db_obj.timestamp = time.time()

        # Request info to update
        if db_obj.service.has_entries():
            sql = db_obj.service.sql_insert(db_obj.person_id,
                                            db_obj.submission_id,
                                            db_obj.origin_node_id,
                                            db_obj.service_id)
            logger.debug(sql)
            self._sql(sql[0], sql[1])

        # Are we updating an existing service?
        if db_obj.service_status.has_entries():
            sql = db_obj.service_status.sql_insert(db_obj.service_id,
                                                   db_obj.submission_id)
            logger.debug(sql)
            self._sql(sql[0], sql[1])

        # Add our submission to the datagbase
        submission_sql = db_obj.submitter_info.sql_insert(db_obj.submission_id,
                                         db_obj.origin_node_id,
                                         db_obj.submitter_id,
                                         timestamp=db_obj.timestamp)
...
class CatanDatabaseObject(object):
    ...
    def pack(self):
        ...
        for a in attributes:
            ...
            if isinstance(value, dict):
                # Filter all None entries so we don't waste network bandwidth
                non_none = dict((k, v) for k, v in value.items() if v is not
                                None)
